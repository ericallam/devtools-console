...
var _objectId = 0;
var RemoteObjectStub = function(object, forceValueType) {
...
['log', 'warn', 'info', 'error'].forEach(function(level) {
    stubbedWindow.console[level] = (function() {
        ...
        consoleFunction.toString = function(){
...
(function() {
    this.evaluate = function(params, sendResult) {
        ...
        if(expression === "this"){
            expression = "stubbedWindow";
        }

        try {
            result = eval.call({}, "with({console: stubbedWindow.console}){\n" + expression + "\n}");
        } catch (e) {
            return sendResult(this.createThrownValue(e, params.objectGroup));
        }

        if(result == window){
          result = stubbedWindow;
        }
    ...
    this.wrapObject = function(object, objectGroup, forceValueType) {
        ...
        try {
            remoteObject = new RemoteObjectStub(object, forceValueType);
        } catch (e) {
            var description = "<failed to convert exception to string>";
            try {
                description = helpers.describe(e);
            } catch (ex) {}
            remoteObject = new RemoteObjectStub(description, forceValueType);
        }
    ...
    this.callFunctionOn = function(params, sendResult) {
        ...
        try {
            var func = eval.call(window, ("(" + params.functionDeclaration + ")"));
...
}).call(RuntimeAgentStub.prototype);
